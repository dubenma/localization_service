function parfor_densePE( qname, dbCutoutname, dbnamesId, posesFromHoloLens, firstQueryId, lastQueryId, params )
...
[~,DBFname,DBFsuffix] = fileparts(dbCutoutname);
...
sequenceLength = 1; %size(dbCutoutname,1);
...
%         end
        ...
        %depth information
        %this_db_matname = fullfile(params.dataset.db.cutout.MatDir, DBFname+DBFsuffix+ params.dataset.db.cutout.matformat);
        this_db_matname = dbCutoutname + ".mat";
...
%             [ P, inls ] = ht_lo_ransac_p3p( tent_ray2d, tent_xdb3d, 5.0*pi/180,max(10000,numel(tent_ray2d))); %1.0*pi/180);
    ...
    if exist(fullfile(params.output.pnp_dense_inlier.dir, QFname), 'dir') ~= 7
        mkdir(fullfile(params.output.pnp_dense_inlier.dir, QFname));
    end
